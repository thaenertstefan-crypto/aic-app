...
5. **ViewTransition-Wrapper:** Der Seiteninhalt von `/wants` und `/schmiede`
   wird je in ein `<ViewTransition>` gehüllt, das `forge-down`/`forge-up` auf
   vertikale Slide-Animationen mappt und `default: "none"` setzt — damit
   animiert **nur** dieser Übergang, alle anderen Navigationen bleiben
   unberührt.
6. **CSS-Keyframes** (in globals.css): vertikale Variante der Doku-Slides
   (`translateY` statt `translateX`), asymmetrisches Timing (Exit schnell ~150 ms,
   Enter etwas langsamer ~210 ms + Delay), leichter Blur/Fade zum Kaschieren.
7. **Reduced Motion:** `@media (prefers-reduced-motion: reduce)` setzt
...
### Verhalten / Fallbacks
- Der Header slidet in diesem Übergang bewusst **mit** dem Inhalt (verstärkt das
  „Scroll"-Gefühl; kein Header-Anchoring nötig).
...
| `components/layout/sub-page-header.tsx` | optional `backTransitionTypes` für Rückweg-Slide (P4) |
